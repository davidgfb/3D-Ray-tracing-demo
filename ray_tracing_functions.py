...
from vector_functions import dot_vecs3,sub_vecs3,sum_vecs3,mul_vec3_n,normalize_vec3
...
def intersect(ro, rd, obj):
    res=0
    if obj[0] == 'sphere':
        res=intersect_sphere(ro, rd, obj[1:])
    elif obj[0] == 'plane':
        res=intersect_plane(ro, rd, obj[1:])
...
def get_color(ind, p):
    res=0
    if ind == 0:
        res= DARKGRAY if (int(p[0] * 2) % 2) == (int(p[2] * 2) % 2) else WHITE
    else:
        res= (BLUE, RED, YELLOW, GREEN, ORANGE, PURPLE)[ind-1]
